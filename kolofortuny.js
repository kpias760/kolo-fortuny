...
kanal do backupu z listą serwerów i kanałów
zmiana, nie musi być koniecznie administratorem, wystarczą inne permissions
dodać f.help
*/
...
var wiadomoscPowitalna = 'Cześć! Dzięki za zaproszenie mnie do swojego serwera. Pozwoliłem sobie stworzyć kanał, na którym będzie można grać w koło fortuny na tym serwerze. Na początku jest on niewidoczny dla innych użytkowników - w każdej chwili możesz to zmienić, jak również nazwę tego kanału i kategorię.\n\n**ZASADY GRY:**\nJeśli kojarzysz program "Koło Fortuny", nie będziesz miał problemów ze zrozumieniem zasad. 6 rund, 6 haseł, kręć kołem i wygrywaj! Obecnie bot jest w fazie "alpha", nie dodałem jeszcze finału - niedługo to się stanie.\n\n**Podstawowe komendy:**\n`f.help` - pomoc\n`f.changelog` - historia wersji bota\n`f.join` - dołącz do gry\n`f.leave` - opuść grę\n\n**Miłej zabawy!**';

hasla = [ ['europejski trybunał praw człowieka','sądy'], ['planeta pozasłoneczna','astronomia'], ['admirał marynarki wojennej','wojsko'], ['ceremonia otwarcia igrzysk olimpijskich','wydarzenie'], ['
...
client.on('guildCreate', (guild) => {
	...
	guild.createChannel('koło-fortuny', 'text', [{
	  id: guild.id,
	  deny: ['VIEW_CHANNEL'],
	  allow: ['SEND_MESSAGES']
	}])
	  .then(channel => nowySerwer(channel))
	  .catch(console.error);
});

function nowySerwer(channel) {
	channel.send(wiadomoscPowitalna);
...
client.on('guildDelete', (guild) => {
	...
	channels.splice(servers.indexOf(guild.id), 1);
	servers.splice(servers.indexOf(guild.id), 1);
});
